...
The completion criteria below are the nine bullets recorded in
...
- **No real Equipment Duty engineering calculation has shipped.**
  ...
  realized in code.
- **No pass/fail duty evaluation with warning threshold/badge
  behavior has shipped.** The 90% / 100% threshold table from spec
  §4.5 is documented but not implemented. The orchestrator does not
  emit `pass` or `fail` rows — only `not_evaluated` / `missing_rating`
  / `not_applicable`. `DutyCheckStatus` is intentionally binary on the
  status axis (`pass` / `fail` / `missing_rating` / `not_applicable` /
  `not_evaluated`); a 90–100% `utilizationPct` would render as a
  `pass` row with a warning badge in the UI — `warning` is not itself
  a row-status literal.
...
| Real Equipment Duty pass/fail evaluation with warning threshold/badge behavior per Equipment Duty spec §4.5 threshold table (`ok` ≤ 90% / `warning` > 90% / `violation` > 100%) — `DutyCheckStatus` stays binary (`pass`/`fail`) on the status axis; the warning band is a UI badge driven by `utilizationPct`, not a third row status | Same gate as above — bundles with the real calculation. |
...
- No real Equipment Duty engineering calculation has shipped; no
  pass/fail duty evaluation with warning threshold/badge behavior
  (`DutyCheckStatus` stays binary on the status axis — `warning` is a
  UI badge, not a row status); numeric duty / rating / utilization /
  margin fields remain `null` on every emitted row.
