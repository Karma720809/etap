...
describe("normalizeShortCircuitResult — synthesized unavailable rows", () => {
  it("synthesizes unavailable rows for AppNetwork buses missing from the wire response", () => {
    // Specific-mode response that only computed eq_bus_mv. eq_bus_lv
    // must come back as `unavailable` with all numerics null.
    const response = makeResponse({
      buses: [
        {
          internalId: "eq_bus_mv",
    ...
    // Unavailable row must not flip top-level status (spec §7.3).
    expect(result.status).toBe("valid");
  });
});
