...
describe("buildAppNetwork — Stage 1 demo fixture", () => {
  it("produces a valid AppNetwork for the canonical demo fixture", () => {
    ...
    // Buses
    const busIds = result.network.buses.map((b) => b.internalId).sort();
    ...
    expect(result.network.sources[0]?.internalId).toBe("eq_util_001");
    expect(result.network.sources[0]?.role).toBe("slack");
    expect(result.network.sources[0]?.busInternalId).toBe("eq_bus_mv_001");
    ...
    expect(result.network.cables[0]?.internalId).toBe("eq_cbl_001");
    expect(result.network.cables[0]?.branchChainEdgeId).toBe("edge_lv_branch_chain_to_motor_bus");
    ...
    expect(result.network.gates[0]?.internalId).toBe("eq_brk_001");

    // Motor collected as a steady-state PQ load
    expect(result.network.motors).toHaveLength(1);
    expect(result.network.motors[0]?.internalId).toBe("eq_motor_001");
    expect(result.network.motors[0]?.busInternalId).toBe("eq_bus_mtr_001");
